...
router.all("/attendance", middle.isAdmin, async (req, res, next) => {
  ...
  if (req.body.user_nick)
    match_users.$match.$and.push({ "users.user_nick": req.body.user_nick });
  if (req.body.region)
    match_works.$match.$and.push({ "works.region": req.body.region });
  if (req.body.isDataOnly === "1")
    match_works.$match.$and.push({ "works.region": { $exists: true } });
            ...
            $match: {
              $expr: {
                $and: [
                  {
                    $eq: [
                      { $substr: ["$work_date", 0, 10] },
                      { $substr: ["$$ref_date", 0, 10] }
                    ]
                  },
                  { $eq: ["$user_id", "$$ref_id"] }
                ]
...
router.put(
  ...
  async (req, res, next) => {
    ...
    if (!member)
      res.json({
        result: 0,
        message: "해당 작업이 없습니다. 관리자에게 문의해주세요."
      });
    else {
      ...
      if (result.nModified === 1) res.json({ result: 1, message: "수정성공" });
      else
...
router.put("/attendance/confirm", middle.isAdmin, async (req, res, next) => {
  ...
  const result = await Work.updateMany(
    { $or: query },
    { $set: { work_confirm: confirm_state } }
  );
  res.json(result);
});
// 근태관리 엑셀다운로드
...
router.post("/attendance/time/:id", middle.isAdmin, async (req, res) => {
  let result = await Work.updateMany(
    { _id: req.params.id },
    {
      $set: {
...
router.put("/work/region/:id/:name", middle.isAdmin, async (req, res, next) => {
  const result = await Region.update(
    { _id: req.params.id },
    { $set: { region_name: req.params.name } }
  );
  res.json(result);
});
// 작업관리 화면, 부서 삭제
...
router.put("/work/work_title/:id/:name", middle.isAdmin, async (req, res) => {
  const result = await WorkTitle.update(
    { _id: req.params.id },
    { $set: { work_title: req.params.name } }
  );
  res.json(result);
});
// 작업관리 화면, 작업명 삭제
...
router.put("/work/remark/:id/:name", middle.isAdmin, async (req, res, next) => {
  const result = await Remark.update(
    { _id: req.params.id },
    { $set: { remark: req.params.name } }
  );
  res.json(result);
});
// 작업관리 화면, 비고 삭제
...
// 작업관리 화면, 출근시간 수정
router.put(
  "/work/start_time/:id/:name",
  middle.isAdmin,
  async (req, res, next) => {
    const result = await StartTime.update(
      { _id: req.params.id },
      { $set: { start_time: req.params.name } }
    );
    res.json(result);
  }
);
...
// 작업관리 화면, 퇴근시간  수정
router.put(
  "/work/end_time/:id/:name",
  middle.isAdmin,
  async (req, res, next) => {
    const result = await EndTime.update(
      { _id: req.params.id },
      { $set: { end_time: req.params.name } }
    );
    res.json(result);
  }
);
...
router.get("/notice/read/:id", middle.isAdmin, async (req, res) => {
  ...
  let user_list;
  if (query.length > 0) {
    user_list = await User.aggregate([{ $match: { $or: query } }]);
  } else {
    user_list = null;
  }
...
router.post("/notice/create", middle.isAdmin, async (req, res) => {
  // 업로드한 파일이 있다면, temp 폴더에서 docs 폴더로 복사
  if (req.body.path) {
    let paths = req.body.path.split(",");
    paths.forEach(function(v) {
      fs.createReadStream("./" + v).pipe(
        fs.createWriteStream("./docs" + v.replace("temps", ""))
      );
    });
  }
...
router.post("/notice/update", middle.isAdmin, async (req, res) => {
  if (req.body.path) {
    ...
    paths.forEach(function(v) {
      if (fs.existsSync("./" + v)) {
        if (v.indexOf("docs/") > -1) return;
        fs.createReadStream("./" + v).pipe(
          fs.createWriteStream("./docs" + v.replace("temps", ""))
        );
      } else {
        // console.log("./" + v + " is not exists");
      }
    });
  }
...
//  공지사항 문서 내 파일 삭제
router.put("/notice/file_detatch", middle.isAdmin, async (req, res) => {
  try {
    let unlink_path = req.body.detatch_path.split(",");
    for (let doc of unlink_path) {
      if (fs.existsSync("." + doc)) {
        fs.unlinkSync("." + doc, err => {
          if (err) {
            console.err(err);
            throw err;
          }
